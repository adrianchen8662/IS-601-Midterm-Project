...
def test_validate_number_ans_with_previous_result():
    prev = Decimal('42')
    assert InputValidator.validate_number('ans', config, previous_result=prev) == prev


def test_validate_number_ans_without_previous_result():
    with pytest.raises(ValidationError, match="No previous result available for 'ans'"):
        InputValidator.validate_number('ans', config, previous_result=None)
